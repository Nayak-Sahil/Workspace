...
AuthRouter.post('/login', [ValidateToken], (req, res) => {
    ...
    if (user.length === 0) {
        ...
        try{
            res.cookie('WS_TOKEN', token, {sameSite: 'None', secure: true, httpOnly: false, path: '/', maxAge: 86400000, domain: process.env.DEV == "true" ? process.env.DEV_FRONT_URL : process.env.PRODUCTION_FRONT_URL});
        }catch(err){
            console.log(err);
        }
